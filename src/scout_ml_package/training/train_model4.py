...
# Preprocess the data
numerical_features = ['TOTAL_NFILES', 'TOTAL_NEVENTS', 'DISTINCT_DATASETNAME_COUNT', 'RAMCOUNT', 'cputime_HS']
categorical_features = ['PRODSOURCELABEL', 'P', 'F', 'CPUTIMEUNIT', 'CORE']
...
tuned_model = pipeline.train_model(processed_train_data, processed_test_data, features_to_train,
                                       'build_cpu_eff', epoch=1, batch=128)
predictions, y_pred = pipeline.regression_prediction(tuned_model, processed_future_data, features_to_train)


# model_storage_path = "ModelStorage/model4/"  # Define the storage path
# model_name = "model4_cpu_eff"  # Define the model name
...
actual_column_name = 'CPU_EFF'  # Change this to match your actual column name
predicted_column_name = 'Predicted_CPU_EFF'  # Change this to match your predicted column name
#plot_directory_name = 'ModelStorage/plots/model4'#'my_plots'  # Optional: specify a custom plots directory
