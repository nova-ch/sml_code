...
training_data = training_data[
   ...
   # (training_data['CPU_EFF'] < 100) &
    ...
    #|
    #((training_data['cputime_HS'] > 10) & (training_data['cputime_HS'] < 3000))
]

training_data = training_data[(training_data['CPUTIMEUNIT'] =='mHS06sPerEvent')]
...
numerical_features = ['TOTAL_NFILES', 'TOTAL_NEVENTS', 'DISTINCT_DATASETNAME_COUNT', 'RAMCOUNT'] #, 'RAMCOUNT'
categorical_features = ['PRODSOURCELABEL', 'P', 'F', 'CPUTIMEUNIT', 'CORE']
...
tuned_model = pipeline.train_model(processed_train_data, processed_test_data, features_to_train,
                                       'build_cputime', epoch=10, batch=256) #build_cputime
