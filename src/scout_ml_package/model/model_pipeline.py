...
import os
from scout_ml_package.model import MultiOutputModel, TrainedModel
from scout_ml_package.model.base_model import ModelTrainer#, ModelPipeline
from scout_ml_package.data import (HistoricalDataProcessor, DataSplitter, ModelTrainingInput, CategoricalEncoder,
                                   TrainingDataPreprocessor, NewDataPreprocessor, LiveDataPreprocessor)
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, confusion_matrix
import numpy as np
import joblib, re, logging
...
class TrainingPipeline:
    ...
    def preprocess_data(self, train_df, test_df, future_data):
        ...
        processed_train_data, encoded_columns, fitted_scaler = training_preprocessor.preprocess(
            train_df, self.numerical_features, self.categorical_features, self.category_list
        )

        new_data_preprocessor = NewDataPreprocessor()

        processed_test_data = new_data_preprocessor.preprocess(
            test_df, self.numerical_features, self.categorical_features, self.category_list, fitted_scaler,
            encoded_columns + self.model_target + ['JEDITASKID']
        )

        processed_future_data = new_data_preprocessor.preprocess(
            future_data, self.numerical_features, self.categorical_features, self.category_list, fitted_scaler,
            encoded_columns + self.model_target + ['JEDITASKID']
        )

        return processed_train_data, processed_test_data, processed_future_data, encoded_columns, fitted_scaler

    def train_model(self, processed_train_data, processed_test_data, features_to_train, build_function_name, epoch, batch):
        #f =  self.categorical_features+ self + self.numerical_features
        #print(f)
        X_train, y_train = processed_train_data[features_to_train], processed_train_data[self.model_target]
        X_val, y_val = processed_test_data[features_to_train], processed_test_data[self.model_target]

        # print("y_train shape:", y_train.shape)
        # print("X_train shape:", X_train.columns)
        output_shape = y_train.shape[1] if y_train.ndim > 1 else 1  # 1D output handling
        ...
        model_ramcount, history_ramcount = trainer.train(X_train, y_train, X_val, y_val, epochs=epoch, batch_size=batch, build_function_name=build_function_name)
        print(model_ramcount.summary())
        return model_ramcount

    def train_classification_model(self, processed_train_data, processed_test_data, features_to_train,
                                   build_function_name, epoch, batch, model_type='binary'):
        X_train, y_train = processed_train_data[features_to_train], processed_train_data[self.model_target]
        X_val, y_val = processed_test_data[features_to_train], processed_test_data[self.model_target]
    ...
    def regression_prediction(self, trained_model, processed_future_data, features_to_train):
        X_test, y_test = processed_future_data[features_to_train], processed_future_data[self.model_target]
        print("y_test shape:", y_test.shape)

        # Evaluate the model
        y_pred = trained_model.predict(X_test)

        pred_names = [f'Predicted_{element}' for element in self.model_target] #['Predicted_RAMCOUNT', 'Predicted_cputime_HS']
    ...
    def classification_prediction(self, trained_model, processed_future_data, features_to_train):
        # Prepare test data
...
class ModelHandlerInProd:
    ...
    def preprocess_data(self, df, numerical_features, category_sequence, unique_elements_categories):
