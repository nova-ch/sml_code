...
def process_row(row, df):
    try:
        if row['CPUTIMEUNIT'] == 'mHS06sPerEvent':
            mhc = ModelHandlerInProd(model_sequence="2", target_name="cputime_low")
            print("Model 2 Loaded")
            temp_df_single_row = df[df['CPUTIMEUNIT'] == 'mHS06sPerEvent']

        else:
            mhc = ModelHandlerInProd(model_sequence="3", target_name="cputime_high")
        ...
        return mhc, temp_df_single_row  # Return both the handler and the filtered DataFrame
...
def make_predictions_for_model(mh, features, numerical_features, category_sequence, unique_elements_categories, input_df):
    """Makes predictions using a given model handler and input DataFrame."""
    try:
        # Preprocess data
        processed_data, features_to_train = mh.preprocess_data(input_df[features], numerical_features,
                                                               category_sequence, unique_elements_categories)
        print(processed_data.head())
        return mh.make_predictions(processed_data, features_to_train)
    except Exception as e:
        logging.error(f"Error processing data with model sequence {mh.model_sequence}: {e}")
        return None
...
base_df[target_var1] = make_predictions_for_model(mh1, features, numerical_features, category_sequence, unique_elements_categories, base_df)
print(base_df)
...
for index, row in base_df.iterrows():
    ...
    predicted_value = make_predictions_for_model(mhc, features, numerical_features, category_sequence, unique_elements_categories, temp_df_single_row)
...
features = ['JEDITASKID'] + numerical_features + category_sequence
base_df[target_var3] = make_predictions_for_model(mh4, features, numerical_features, category_sequence, unique_elements_categories, base_df)
...
features = ['JEDITASKID'] + numerical_features + category_sequence
base_df[target_var4] = make_predictions_for_model(mh5, features, numerical_features, category_sequence, unique_elements_categories, base_df)
print(base_df)
...
from scout_ml_package.data.fetch_db_data import fetch_task_param, get_db_connection

def test_db_connection(conn):
    try:
        # Simple test query
        cursor = conn.cursor()
        cursor.execute("SELECT 1 FROM dual")  # For Oracle, 'dual' is a dummy table
