    ...
    try:
        ...
        for jeditaskid in demo_task_listener(sample_tasks, delay=10):
            ...
            if r is not None:
                ...
                logger.debug(
                    f"Processing completed successfully. Results:\n{base_df['JEDITASKID'].values}"
                )
                ...
                def check_predictions(df, column, range_tuple):
                    ...
                    if (
                        df[column].min() < min_val
                        or df[column].max() > max_val
                    ):
                        raise ValueError(
                            f"Predictions for {column} are outside the acceptable range of {range_tuple}"
                        )
                ...
                base_df["RAMCOUNT"] = processor.make_predictions_for_model(
                    "1", features, base_df
                )
                check_predictions(
                    base_df, "RAMCOUNT", acceptable_ranges["RAMCOUNT"]
                )
                ...
                for _, row in base_df.iterrows():
                    model_sequence, temp_df = processor.process_row(
                        row, base_df
                    )
                    if model_sequence is not None:
                        prediction = processor.make_predictions_for_model(
                            model_sequence, features, temp_df
                        )
                        predicted_values.append(prediction)

                base_df["cputime_HS"] = np.array(predicted_values)
                check_predictions(
                    base_df, "cputime_HS", acceptable_ranges["cputime_HS"]
                )
                ...
                base_df["CPU_EFF"] = processor.make_predictions_for_model(
                    "4", features, base_df
                )
                check_predictions(
                    base_df, "CPU_EFF", acceptable_ranges["CPU_EFF"]
                )
                ...
                base_df["IOINTENSITY"] = processor.make_predictions_for_model(
                    "5", features, base_df
                )
